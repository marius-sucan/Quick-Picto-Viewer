...
WIA_AcquireImage() {
   ...
   If (wiaImg.width>32100 || wiaImg.height>32100)
      ScaledWiaIMG := WIA_ScaleImage(wiaImg, 32100, 32100, 1)

   PicObj := ScaledWiaIMG.Height ? WIA_GetImageBitmap(ScaledWiaIMG) : WIA_GetImageBitmap(wiaImg)
   If (PicObj.Handle)
   {
...
; ======================================================================================================================
WIA_ScaleImage(ImgObj, PxWidth, PxHeight, PreserveRatio:=0) {
   If (ComObjType(ImgObj, "Name") <> "IImageFile")
      Return False
   If !WIA_IsInteger(PxWidth, PxHeight) || ((PxWidth < 1) && (PxHeight < 1))
      Return False

   KeepRatio := (PxWidth<1 || PxHeight<1 || PreserveRatio=1) ? 1 : 0
   ; msgbox, % pxwidth " x " pxheight " -- " KeepRatio
   ImgProc := WIA_ImageProcess()
...
WIA_ImageProcess() {
   Static ImageProcess
   If !ImageProcess
      Try ImageProcess := ComObjCreate("WIA.ImageProcess")
