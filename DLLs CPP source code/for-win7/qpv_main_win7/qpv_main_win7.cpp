// qpv_main_win7.cpp : Defines the initialization routines for the DLL.
//

#include "stdafx.h"
// #include "qpv_main_win7.h"
...
int RGBtoGray(int sR, int sG, int sB, int alternateMode) {
  ...
  if (alternateMode==1)
     return myround((float)(sR*0.299 + sG*0.587 + sB*0.114)); // weighted grayscale conversion
  ...
  return myround(L/2); // return derived luminosity in pseudo-LAB color space
}
...
// https://zschuessler.github.io/DeltaE/demos/
  ...
  double xGX = 0.5 * ( 1 - sqrt( pow(xCX,7) / ( pow(xCX,7) + pow(25.0f,7) ) ) );
  ...
  double xRC = 2 * sqrt( pow(xCY, 7) / ( pow(xCY, 7) + pow(25.0f, 7) ) );   // based on C-Bar Prime
...
std::array<double,3> RGBtoLAB(int sR, int sG, int sB) {
  // https://getreuer.info/posts/colorspace/index.html
  // http://www.easyrgb.com/en/math.php
  // sR, sG and sB (Standard RGB) input range = 0 ÷ 255
...
DLL_API int DLL_CALLCONV FillImageHoles(int *imageData, int w, int h, int newColor) {
    fnOutputDebug("FillImageHoles newColor = " + std::to_string((long double)newColor));
...
double calcArrayAvgMedian(std::array<double, 64> givenArray, int modus) {
    ...
    if (modus==1) // median
    {
        ...
        if (size % 2 == 0) {
            return (givenArray[myround(size / 2 - 1)] + givenArray[myround(size / 2)]) / 2;
        }

        return givenArray[floor(size / 2.0f)];
    } else
    {
...
std::array<double, 32> calculateDCT(const std::array<double, 32> &matrix, int col, int loopu) {
    // int size = 32;
...
std::array<UINT, 3> adaptImageGivenSize(UINT keepAratio, UINT ScaleAnySize, UINT imgW, UINT imgH, UINT givenW, UINT givenH) {
  ...
  } else if (keepAratio==1) 
  {
     if (imgW>givenW || imgH>givenH || ScaleAnySize==1)
     {
         ...
         {
            size[0] = givenW;
            size[1] = myround(size[0] / PicRatio);
            if (size[1]>givenH)
            {
               size[1] = (imgH <= givenH) ? givenH : imgH;
               size[0] = myround(size[1] * PicRatio);
            }
...
DLL_API Gdiplus::GpBitmap* DLL_CALLCONV LoadWICimage(int threadIDu, int noBPPconv, int givenQuality, UINT givenW, UINT givenH, UINT keepAratio, UINT ScaleAnySize, UINT givenFrame, int doFlip, int doGrayScale, const wchar_t *szFileName, UINT *resultsArray) {
    ...
    if (szFileName)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            resultsArray[4] = myround((dpix + dpiy)/2);
