...
static unsigned char blend_sqrt_byte[256];     // byte -> round(sqrt(i/255)*255), for integer soft light
...
// ============================================================================
// CalculateGeminiNewBlendModes - Ultra-fast integer LUT-based blend mode computation
// ============================================================================
static unsigned char gemini_blend_lut[23][256][256];
static bool gemini_lut_initialized = false;

RGBAColor CalculateGeminiNewBlendModes(
  RGBAColor Orgb,
  RGBAColor Brgb,
  int blendMode,
  int flipLayers,
  ...
  int opacity) {
    ...
    // Fallback to SIMD optimized float function for linear gamma or special modes
    if (linearGamma == 1 || blendMode == 23 || blendMode == 24 || blendMode == 100) {
        return CalculateNewBlendModes(Orgb, Brgb, blendMode, flipLayers, linearGamma, keepAlpha, bpp, opacity);
    }

    if (!gemini_lut_initialized) {
        for (int mode = 0; mode < 23; ++mode) {
            ...
            for (int o = 0; o < 256; ++o) {
                for (int b = 0; b < 256; ++b) {
                    ...
                    float fT = 0.0f;
                    switch(mode) {
                        case 0: case 25: fT = fO; break;
                        case 1: fT = (fO < fB) ? fO : fB; break;
                        case 2: fT = fO * fB; break;
                        case 3: fT = fO + fB - 1.0f; break;
                        case 4: fT = (fO > 0.0f) ? (1.0f - ((1.0f - fB) / fO)) : 0.0f; break;
                        case 5: fT = (fO > fB) ? fO : fB; break;
                        case 6: fT = 1.0f - ((1.0f - fB) * (1.0f - fO)); break;
                        case 7: fT = fO + fB; break;
                        case 8: fT = (fO < 0.5f) ? (2.0f * fO * fB) : (1.0f - (2.0f * (1.0f - fO) * (1.0f - fB))); break;
                        case 9: {
                            float sqrt_fB = (fB > 0.0f) ? sqrt(fB) : 0.0f;
                            fT = (fO < 0.5f) ? ((1.0f - 2.0f * fO) * (fB * fB) + 2.0f * fB * fO) : (2.0f * fB * (1.0f - fO) + sqrt_fB * (2.0f * fO - 1.0f));
                            break;
                        }
                        case 10: fT = (fB < 0.5f) ? (2.0f * fO * fB) : (1.0f - (2.0f * (1.0f - fO) * (1.0f - fB))); break;
                        case 11: fT = (fO <= (1.0f - fB)) ? 0.0f : 1.0f; break;
                        case 12: fT = fB + (2.0f * fO) - 1.0f; break;
                        case 13: fT = (fO < 1.0f) ? (fB / (1.0f - fO)) : 1.0f; break;
                        case 14: fT = (fO < 0.5f) ? ((fO > 0.0f) ? (1.0f - (1.0f - fB) / (2.0f * fO)) : 0.0f) : ((fO < 1.0f) ? (fB / (2.0f * (1.0f - fO))) : 1.0f); break;
                        case 15: fT = (fB + fO) * 0.5f; break;
                        case 16: fT = (fO > 0.0f) ? (fB / fO) : 1.0f; break;
                        case 17: fT = fO + fB - 2.0f * (fO * fB); break;
                        case 18: fT = abs(fB - fO); break;
                        case 19: fT = fB - fO; break;
                        case 22: fT = 1.0f - abs(fO - fB); break;
                        default: fT = fO; break;
                    }
                    if (fT < 0.0f) fT = 0.0f;
                    if (fT > 1.0f) fT = 1.0f;
                    gemini_blend_lut[mode][o][b] = (unsigned char)(fT * 255.0f + 0.5f);
                }
            }
        }
        gemini_lut_initialized = true;
    }
    ...
    int rT, gT, bT;
    if (blendMode == 20) {
        int lO = getGrayscale(Orgb.r, Orgb.g, Orgb.b);
        int lB = getGrayscale(Brgb.r, Brgb.g, Brgb.b);
        rT = lO + Brgb.r - lB;
        gT = lO + Brgb.g - lB;
        bT = lO + Brgb.b - lB;
        rT = (rT < 0) ? 0 : (rT > 255) ? 255 : rT;
        gT = (gT < 0) ? 0 : (gT > 255) ? 255 : gT;
        bT = (bT < 0) ? 0 : (bT > 255) ? 255 : bT;
    } else if (blendMode == 21) {
        int lO = getGrayscale(Orgb.r, Orgb.g, Orgb.b);
        int lB = getGrayscale(Brgb.r, Brgb.g, Brgb.b);
        rT = lB - lO + Brgb.r + (Orgb.r * 51) / 255;
        gT = lB - lO + Brgb.g + (Orgb.g * 51) / 255;
        bT = lB - lO + Brgb.b + (Orgb.b * 51) / 255;
        rT = (rT < 0) ? 0 : (rT > 255) ? 255 : rT;
        gT = (gT < 0) ? 0 : (gT > 255) ? 255 : gT;
        bT = (bT < 0) ? 0 : (bT > 255) ? 255 : bT;
    } else {
        int mode = (blendMode < 23) ? blendMode : 0;
        rT = gemini_blend_lut[mode][Orgb.r][Brgb.r];
        gT = gemini_blend_lut[mode][Orgb.g][Brgb.g];
        bT = gemini_blend_lut[mode][Orgb.b][Brgb.b];
    }

    const bool mix = (keepAlpha != 1 || blendMode >= 22 || blendMode < 2);
    if (Brgb.a < 255 && blendMode > 0 && mix) {
        int w_fixed = 255 - Brgb.a;
        rT = (w_fixed * Orgb.r + Brgb.a * rT) / 255;
        gT = (w_fixed * Orgb.g + Brgb.a * gT) / 255;
        bT = (w_fixed * Orgb.b + Brgb.a * bT) / 255;
    }
    ...
    const int w_sa_fixed = (sa * 65536) / resultA;
    const int w_da_fixed = 65536 - w_sa_fixed;

    int finalR = (w_sa_fixed * rT + w_da_fixed * Brgb.r + 32768) >> 16;
    int finalG = (w_sa_fixed * gT + w_da_fixed * Brgb.g + 32768) >> 16;
    int finalB = (w_sa_fixed * bT + w_da_fixed * Brgb.b + 32768) >> 16;

    RGBAColor result;
    result.r = (finalR < 0) ? 0 : (finalR > 255) ? 255 : finalR;
    result.g = (finalG < 0) ? 0 : (finalG > 255) ? 255 : finalG;
    result.b = (finalB < 0) ? 0 : (finalB > 255) ? 255 : finalB;
    result.a = resultA;
    if (keepAlpha == 1 && oA != -1)
        result.a = oA;
    return result;
